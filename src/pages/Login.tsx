...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        localStorage.setItem('itms_token', data.token);
        localStorage.setItem('itms_user', JSON.stringify(data.user));
        navigate(data.user.defaultPortal);
      } else {
        ...
        <div className="bg-white py-8 px-4 shadow-sm sm:rounded-xl sm:px-10 border border-slate-200">
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              ...
              <p className="mt-3 text-xs text-slate-500 text-center">
                Demo Auth: Use 'admin' for Super Admin, 'it' for IT Team, anything else for Employee.
              </p>
            </div>
          </form>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-1 gap-3">
              <button
                type="button"
                className="w-full inline-flex justify-center items-center py-2 px-4 border border-slate-300 rounded-md shadow-sm bg-white text-sm font-medium text-slate-500 hover:bg-slate-50"
              >
